    ...
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        <ChartCard title="Revenue" subtitle="Last 7 months">
          <AreaChart data={revenueData}>
            ...
            <XAxis dataKey="month" tick={{ fill: '#6a6a7a', fontSize: 12 }} axisLine={false} tickLine={false} />
            <YAxis tick={{ fill: '#6a6a7a', fontSize: 12 }} axisLine={false} tickLine={false} tickFormatter={(v) => `$${(v / 1000).toFixed(0)}k`} />
            <Tooltip {...chartTooltipStyle} formatter={(value) => [`$${Number(value).toLocaleString()}`, 'Revenue']} />
            <Area type="monotone" dataKey="revenue" stroke="#c9a87c" fill="url(#revenueGradient)" strokeWidth={2} />
          </AreaChart>
        </ChartCard>
        ...
        <ChartCard title="Orders" subtitle="Monthly order volume">
          <BarChart data={revenueData}>
            <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.04)" />
            <XAxis dataKey="month" tick={{ fill: '#6a6a7a', fontSize: 12 }} axisLine={false} tickLine={false} />
            <YAxis tick={{ fill: '#6a6a7a', fontSize: 12 }} axisLine={false} tickLine={false} />
            <Tooltip {...chartTooltipStyle} />
            <Bar dataKey="orders" fill="#7c9ec9" radius={[6, 6, 0, 0]} />
          </BarChart>
        </ChartCard>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-10">
        <div className="bg-waxe-card backdrop-blur-md border border-waxe-border rounded-2xl p-5">
          <h3 className="text-sm font-semibold text-waxe-text mb-4">Genre Breakdown</h3>
        ...
        <div className="bg-waxe-card backdrop-blur-md border border-waxe-border rounded-2xl p-5">
          <h3 className="text-sm font-semibold text-waxe-text mb-4">Condition Distribution</h3>
          <div className="space-y-3">
            {conditionBreakdowns.map((c) => (
              <div key={c.condition} className="flex items-center gap-3">
                ...
                <div className="flex-1 h-2 bg-waxe-surface rounded-full overflow-hidden">
                  <div className="h-full bg-[#7c9ec9] rounded-full" style={{ width: `${c.percentage}%` }} />
                </div>
      ...
      {/* Market Trends */}
      <h2 className="text-lg font-semibold text-waxe-text mb-4">Market Trends</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        <ChartCard title="Genre Demand" subtitle="Search & purchase interest by genre">
          <LineChart data={genreDemandData}>
            ...
            <YAxis tick={{ fill: '#6a6a7a', fontSize: 12 }} axisLine={false} tickLine={false} />
            <Tooltip {...chartTooltipStyle} />
            <Line type="monotone" dataKey="jazz" stroke="#c9a87c" strokeWidth={2} dot={false} name="Jazz" />
            <Line type="monotone" dataKey="rock" stroke="#7c9ec9" strokeWidth={2} dot={false} name="Rock" />
            <Line type="monotone" dataKey="hiphop" stroke="#7cb89a" strokeWidth={2} dot={false} name="Hip-Hop" />
            <Line type="monotone" dataKey="electronic" stroke="#9b7cc9" strokeWidth={2} dot={false} name="Electronic" />
            <Line type="monotone" dataKey="soul" stroke="#c97c9e" strokeWidth={2} dot={false} name="Soul" />
          </LineChart>
        </ChartCard>
        ...
        <ChartCard title="Price Trends by Condition" subtitle="Average sale price by rating">
          <LineChart data={priceTrendData}>
            ...
            <Line type="monotone" dataKey="c10" stroke="#c9a87c" strokeWidth={2} dot={false} name="10 — Sealed" />
            <Line type="monotone" dataKey="c9" stroke="#7c9ec9" strokeWidth={2} dot={false} name="9 — Pristine" />
            <Line type="monotone" dataKey="c7" stroke="#7cb89a" strokeWidth={2} dot={false} name="7 — Very Good" />
            <Line type="monotone" dataKey="c5" stroke="#6a6a7a" strokeWidth={2} dot={false} name="5 — Fair" />
          </LineChart>
        </ChartCard>
      </div>

      {/* Vinyl-Relevant Releases */}
      <div className="bg-waxe-card backdrop-blur-md border border-waxe-border rounded-2xl p-5 mb-10">
        <h3 className="text-sm font-semibold text-waxe-text mb-1">Vinyl-Relevant Releases</h3>
        <p className="text-xs text-waxe-text-muted mb-4">Trending albums with vinyl release potential based on streaming data</p>
        <div className="space-y-3">
          {[
            ...
            <div key={i} className="flex items-center justify-between py-2 px-3 rounded-lg hover:bg-waxe-surface/30 transition-colors">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded bg-waxe-surface border border-waxe-border flex items-center justify-center text-xs font-bold text-waxe-text-muted">
      ...
      {/* Restocking */}
      <h2 className="text-lg font-semibold text-waxe-text mb-4">Restocking</h2>

      <ChartCard title="Inventory Velocity" subtitle="Items sold vs. added per week" className="mb-6">
        <BarChart data={velocityData}>
          <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.04)" />
          <XAxis dataKey="week" tick={{ fill: '#6a6a7a', fontSize: 12 }} axisLine={false} tickLine={false} />
          <YAxis tick={{ fill: '#6a6a7a', fontSize: 12 }} axisLine={false} tickLine={false} />
          <Tooltip {...chartTooltipStyle} />
          <Bar dataKey="sold" fill="#c9a87c" radius={[4, 4, 0, 0]} name="Sold" />
          <Bar dataKey="added" fill="#7c9ec9" radius={[4, 4, 0, 0]} name="Added" />
        </BarChart>
      </ChartCard>
