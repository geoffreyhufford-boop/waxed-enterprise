  ...
  return (
    <div className="flex items-center gap-2 py-1.5">
      ...
      <div className="shrink-0 text-right">
        <p className="text-[11px] font-bold text-waxe-text font-mono">${record.marketValue}</p>
        <p className="text-[10px] text-waxe-text-muted">{record.genre}</p>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="flex-1 min-w-0">
      <div className="flex items-center gap-2 mb-2">
        <p className="designation-tag">{label}</p>
    ...
    <div className="bg-waxe-card border-2 border-waxe-border rounded-none p-5 clip-card-tr-bl">
      ...
      <div className="flex items-start justify-between mb-4">
        <div>
          ...
          <p className="text-[11px] text-waxe-text-muted">{proposal.storeB.location} &middot; {proposal.createdAt}</p>
        </div>
      ...
      <div className="flex gap-3 items-stretch mb-4">
        ...
        <div className="shrink-0 flex flex-col items-center justify-center px-1">
          ...
          <div className="my-2 glyph-box" style={{ width: '36px', height: '36px', fontSize: '16px' }}>
            ⇄
          </div>
          <div className="w-px flex-1 bg-waxe-border" />
        </div>
      ...
      {proposal.status === 'pending' && (
        <div className="flex gap-2">
          <button onClick={() => onAccept?.(proposal.id)} className="btn-primary text-[11px] px-4 py-2 flex-1">
            Accept Swap
          </button>
          <button onClick={() => onDecline?.(proposal.id)} className="btn-ghost text-[11px] px-4 py-2">
            Decline
          </button>
          <button onClick={() => onViewDetails?.(proposal.id)} className="btn-secondary text-[11px] px-4 py-2">
            Details
          </button>
        </div>
      )}
      {proposal.status === 'accepted' && (
        <div className="flex gap-2">
          <button onClick={() => onViewDetails?.(proposal.id)} className="btn-secondary text-[11px] px-4 py-2 flex-1">
            View Details
          </button>
        </div>
      )}
